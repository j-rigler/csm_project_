...
data_folder = './data/'            # folder with data
gemini = './gemini_output/'              # folder for gemini results
...
XS = pd.DataFrame(index = ai_index, columns = ['ai_e_index'])
XS.index.names = ['area','item']

# Find the shocked country and item in the index
shock_ids = list(range(len(ai_e_index)))
...
for t in range(tau): # Outer loop: iterates through simulation time steps (0 to 9)

    # 1. Determine the actual year for the current simulation timestep 't'
    current_sim_year = years[t]

    # 2. Filter 'food_production_data' to get only the relevant shocks for this specific year
    shocks_for_this_year = food_production_data[food_production_data['Year'] == current_sim_year]

    # 3. Calculate current output 'o' *before* applying specific shocks for this timestep
    ...
    # Apply start stock if it's the first iteration
    if t == 0:
        o = o + xstartstock

    # 4. Apply the specific shocks for this year to the 'o' vector
    #    Iterate through the relevant shocks, find their corresponding position in 'o',
    #    and apply the 'Relative Difference' from food_production_data.
    for index, row in shocks_for_this_year.iterrows():
        area_name = row['Area']
        item_name = row['Item']
        relative_diff_value = row['Relative Difference'] # This is the specific shock magnitude

        try:
            # Get the integer index for this (Area, Item) pair in the global 'ai_index'
            # 'o' is implicitly aligned with 'ai_index'
            idx_pos_in_o = ai_index.get_loc((area_name, item_name))

            # Apply the shock to the corresponding position in 'o'
            # The 'relative_diff_value' is the specific shock for this (Area, Item, Year)
            o[idx_pos_in_o] = (1 + relative_diff_value) * o[idx_pos_in_o]

        except KeyError:
            # This warning helps you identify if any (Area, Item) from food_production_data
            # is not present in your main 'areas' or 'items' lists used for 'ai_index'.
            print(f"Warning: ({area_name}, {item_name}) from food_production_data (Year: {current_sim_year}) not found in global ai_index. Skipping shock for this pair.")

    # ... (rest of your loop for 'h', 'xs', 'rl', 'al', compensation logic, timing, etc.)
    # All the code below the shock application will remain the same.
    ...
    # Check for events
    # ... (rest of your compensation logic, unchanged)
    ...
    # save
    if compensation:
        XS.to_csv(gemini + 'combined_shocks' + '_comp.csv')
    else:
        XS.to_csv(gemini + 'combined_shocks' + '_no_comp.csv')
