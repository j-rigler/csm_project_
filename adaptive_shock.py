...
### SIMULATION: SHOCK ADAPTATION ###
                        ...
                        # Prepare storage
XS                = pd.DataFrame(index = ai_index, columns = ['combined_shocks'])
XS.index.names    = ['area','item']
XS.columns.names  = ['scenario']

                        # Find the shocked countries and items in the index
sector_ids = [list(ai_index.values).index(sector) for sector in shock_scenario]
...
for t in range(tau):
    ...
    for sector_id in sector_ids:
        o[sector_id] = shock_scaling[t] * o[sector_id]
...
if compensation:
    ...
    XS.to_csv(output_folder + 'combined_shocks' + '_no_comp.csv')
